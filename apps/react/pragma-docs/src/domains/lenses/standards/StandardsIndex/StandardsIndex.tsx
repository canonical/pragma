      ...
      <div className="standards-groups" data-region="canvas">
        ...
        {groupUris.map((classUri) => (
          <section
            ...
            key={classUri}
          >
            <h2 id={`${groupAnchorId(classUri)}-title`}>
              {groups.get(classUri)?.title}
            </h2>
              ...
              {(groups.get(classUri)?.nodes ?? []).map((node) => (
                <li key={node.uri}>
                  {/* Link text is `_meta.title`, which is TOTAL — the
                      provider computes a fallback chain ending in the IRI
                      local name, then the whole IRI when there is no local
                      name, so there is no `??` arm here and no fabricated
                      title.
