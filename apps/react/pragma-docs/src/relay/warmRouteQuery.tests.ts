...
describe("warmRouteQuery", () => {
  it("is a no-op when no prefetch environment is published (the SSR posture)", () => {
    // `not.toThrow()` alone would pass for an empty function body AND for one
    // that warmed the wrong thing — "does nothing" and "does the wrong thing"
    // are the two outcomes this case has to tell apart, so the assertion is
    // that no fetch was attempted, not merely that nothing blew up.
