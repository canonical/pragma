...
describe("JourneyTable group interaction", () => {
  ...
  it("scopes each group header to the ROW group it opens", () => {
    // Raised in review. A group header cell sits inside the `<tbody>` whose
    // journey rows it names, and a `<tbody>` is a row group — so
    // `scope="rowgroup"`. `scope="colgroup"` states the opposite
    // relationship, and states it confidently: assistive technology would
    // announce the group label as a header for the COLUMNS.
    renderStateful();

    const headers = [
      ...document.querySelectorAll(".journey-table-group-header"),
    ];
