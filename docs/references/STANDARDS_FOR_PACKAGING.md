...
Releases are triggered by a manual `workflow_dispatch` on the `tag.yml` workflow. The workflow builds and tests first, then uses Lerna's `--conventional-commits` flag to determine version bumps from commit messages, generate changelogs, and create annotated git tags. Publishing uses [OIDC trusted publishing](https://docs.npmjs.com/trusted-publishers): the publish job has `id-token: write` permission and GitHub mints a short-lived npm token at publish time — no `NPM_AUTH_TOKEN`/`NODE_AUTH_TOKEN` secret is used. Each package must have a trusted publisher configured on npmjs.com (repo `canonical/pragma`, workflow `tag.yml`). A `DEPLOY_KEY` SSH key allows the version job to push commits and tags back to the repo.
...
| M8.4 | npm auth uses OIDC trusted publishing (`id-token: write`, no token secret) | `permissions: id-token: write` on publish job + trusted publisher set per package |
...
jobs:
  ...
  publish:
    ...
    runs-on: ubuntu-latest
    needs: version
    # OIDC trusted publishing: GitHub mints a short-lived npm token at publish time.
    permissions:
      id-token: write
      contents: read
    steps:
      ...
      - name: Publish packages
        run: lerna publish --yes --no-private --access public from-package
```
...
2. Configure [OIDC trusted publishing](https://docs.npmjs.com/trusted-publishers) for each published package on npmjs.com (repo `canonical/pragma`, workflow `tag.yml`); the publish job uses `id-token: write`, so no npm token secret is needed.
