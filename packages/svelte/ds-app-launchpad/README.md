...
This package's own sheets — the reset, the token shims, the modifier families and the element rules in `src/lib/styles/` — are in `ds.components.app` with the component sheets, because that is where they were in the cascade before: all of them were unlayered together, so their order among themselves is unchanged. Two files are deliberate exceptions. `src/lib/styles/font-faces.css` holds only `@font-face` registrations, which no layer sorts. `src/lib/styles/ds-shim.css` also writes into `ds.modifiers`, where the design-tokens values it stands in for are generated; that block is declared first in the file so the two layers open in the order the statement gives them.

Note that this package's CSS entry deliberately does not import `@canonical/styles` yet (see the TODO in `src/lib/index.css`), so an application that loads only this package gets no `@layer` order statement and its layer order is first-appearance order. That is fine today — nothing in this package writes into a layer that a design-tokens sheet also writes into — but it is the reason the entry's TODO matters.
