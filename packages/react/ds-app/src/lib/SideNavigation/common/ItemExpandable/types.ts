...
type OwnProps = {
   ...
   * whether the active item is among this item's children (SPEC.md §5) so
   * the current page's ancestor chain starts expanded.
   */
   ...
   * stays open so the active row remains visible (SPEC.md §5). Defaults
   * to `false`.
   */
