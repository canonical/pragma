...
export interface ContextSwitcherItem {
  ...
  name: string;
  /**
   * Optional route for the context. Data polymorphism (the menu's APG
   * constraint means there is no render target for a `LinkComponent` inside
   * a `role="menu"` — every row is a `<div role="menuitem">`): when given,
   * the url rides through `onContextChange`, and the consumer routes with
   * their own router wrapper (that callback IS the custom router adapter).
