...
`renderToStream()` loads the URL into the router, renders `ServerRouter`, and
returns:
...
`ServerRouter` is the server-oriented convenience component that combines
`RouterProvider` and `Outlet`.

```tsx
<ServerRouter fallback={<p>Loading route…</p>} router={router} />
```
...
- `ServerRouter` — convenience wrapper that combines `RouterProvider` and
  `Outlet` for SSR.
...
- `ServerRouterProps` — `Outlet` props plus a router instance.
