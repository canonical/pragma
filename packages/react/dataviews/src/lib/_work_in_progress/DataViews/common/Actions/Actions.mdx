...
The count is the selection's, across pages — not the rows on screen — and an action acts on every selected target. The deselect button is named with what it clears ("Deselect 2 rows").
