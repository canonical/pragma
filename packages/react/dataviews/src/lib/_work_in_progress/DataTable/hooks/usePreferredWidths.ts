...
import { readBounds, spellWidthKey } from "../common/utils/index.js";
...
export default function usePreferredWidths(
  ...
  useLayoutEffect(() => {
    ...
    const implied = (id: string, sizing: ColumnSizing): ColumnSizing | null => {
      const width = views.state.get().presentation[spellWidthKey(id)];
      if (typeof width !== "number" || !Number.isFinite(width) || width < 0) {
        return null;
      }
      const { min, max } = readBounds(sizing);
    ...
    const stopLayout = layout.state.subscribe(() => {
      ...
      for (const [id, declaredSizing] of declared) {
        ...
        const agrees =
          override === undefined
            ? sizing === null
            : sizing !== null && areSizingsEqual(override, sizing);
        if (!agrees) {
          // A reset is kept as null, over a width a view was saved with.
          patch[spellWidthKey(id)] =
            override?.kind === "fixed" ? override.px : null;
        }
      }
      seen = overrides;
