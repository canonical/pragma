...
React bindings for Canonical collection views, built on `@canonical/dataviews-core`: the `DataViews` root with its connected `Search`, `Filters`, `SortPanel`, `Actions`, `Pagination` and `SavedViews` parts, the provider context, the four scoped observation hooks, the `DataTable` renderer and the `PaginationBar`.
...
- **`DataViews`** — the root component: mounts a provider created by `createDataViewsProvider` (from `@canonical/dataviews-core`) as the collection's context, observes it for as long as it is mounted — which starts the provider's source, location, presentation and saved views, ref-counted, so a table inside the root observing too costs nothing and a StrictMode rehearsal leaves a working provider — and owns the root's filter records. Throws unless the provider is one `createDataViewsProvider` built. Nothing is wired in an effect by the application: it builds the provider once, in state, and mounts.
- **Messages and announcements** — every word a part renders or announces comes from one messages record, `DataViewsMessages` (from `@canonical/dataviews-core`), in English unless the application says otherwise. The root takes `messages`: any of them, merged over English, or all of them — annotate a whole replacement as `DataViewsMessages` and a missing message fails to compile. A message stating a fact is a function of it — a count, a position, a name, a reason — and words its own plurals and ordinals. Field and option labels are not messages. A standalone `DataTable` or `PaginationBar` is its own root and takes `messages` itself; the connected parts take their root's. What a command did that has no place on screen — a sort's new order, why it changed nothing, a column hidden, shown or moved — is said through one polite live region per root, rendered after its children: what is said in one moment is spoken as one announcement — words repeated in that moment said once, and an outcome with one latest answer, such as what the rows are ordered by, standing where the last one stood — the same words said again in a later moment are spoken again, and a second root over the same provider says nothing of the first root's commands. Status regions that belong to a place — the pagination summary, the table's status row, a filter's feedback, the saved views' status — speak for themselves and are never repeated.
- **`useDataViews(collection)`** — the typed collection scope for custom children: the collection's state channel, its rows, the refused URL clauses, selection, saved views, presentation, this root's filter handles and the bounded commands, `runAction` and `refusals` among them. The module-scope collection is the type and identity witness: it must be the one the enclosing root's provider was built over, so a custom child lives at module scope with no provider prop, and a nested root over another collection rejects it rather than answering with the wrong records typed as the right ones.
- **`useDataViewsValue(channel, select?)`** — observe one channel; re-render only when it publishes, or only when the part `select` picks changes. A selector returns a part of the snapshot, never a value it mints.
...
- **`DataViews.Search`** — the connected search: one labelled native search input over the applied search. Every edit applies as it is typed through `setSearch`, and a search replaces the history entry rather than pushing one, so typing never floods history and Back restores the applied search into the input. It reads the applied search alone, so a result arriving re-renders no search box; it throws where the source declares no search, as SavedViews does without a store. At baseline it is a GET form carrying the rest of the query hidden.
- **`DataViews.Filters`** — the connected query-editing part. Which fields it offers and which operators each accepts come from the provider — its collection's schema, and the capabilities its source declares — so it never offers a restriction the source would refuse, takes no query props, and has no draft query to keep in step with the applied one. An edit the source refuses keeps the restriction in force and says why beside the control. At baseline it is a GET form whose controls are named as the URL spells their clauses, a number bound a native number input carrying the schema's bounds, with the rest of the query hidden beside them; once scripting is enabled every edit applies as it is made and a submission is intercepted.
- **`DataViews.SortPanel`** — the reader's ordering as a list: each term in precedence order with its direction, moved up, moved down or removed through `setSort`. The path to an ordering of several terms that needs no Shift key. While the reader states no term it says what orders the rows instead. Without scripts each move and removal is a link to the ordering it leads to, from page one. Deliberately rough; its design comes later.
...
- **`DataViews.SavedViews`** — the connected saved views: which view is open, whether the query has moved from it, and switching, saving, saving as, renaming, reverting and deleting, over the store the provider was given; its notices also say when the provider's presentation is not being saved. Throws when the provider has no view store; without JavaScript it says only that views are unavailable.
- **`PaginationBar`** — the bar beneath a collection's rows, observing its provider like the table does: the page size, a summary of the rows on screen out of the filtered total, a page select and first, previous, next and last page controls. Its destinations are the ones the collection can actually reach: a source that counts the rows the window pages over exactly gets a page total and a last page; one that counts them loosely or not at all gets a Next taken from the page's own word that more exists, or, failing that, offered only while the page is full; a source reaching its pages only through tokens is paged by those tokens, without a page total or a last page to jump to; and a pending or failed replacement claims no total at all. At baseline every page it can reach is a real link the provider's encoder spelled — through the consumer's router `Link` where `LinkComponent` names one, against the design system's shared link contract — and the page size and the page select each sit in a GET form carrying the rest of the query; once scripting is enabled a plain click on the intrinsic anchor or a submission moves the window in place, and controls only the enhancement can drive wait for it.
...
- **`DataTable`** — the row renderer: div rows over ARIA table roles, one shared column track list, sorting, selection and resizing. It observes its provider for as long as it is mounted, so a standalone table is a complete screen on its own. Which columns it shows, in what order and at what width is the provider's presentation: the table renders the arrangement in force and writes a resize back to it, so every table on one provider agrees, and a column may declare `hideable: false`. A column's `sortable` is honoured only on a field the provider's source declares sortable — the same declaration `DataViews.Filters` reads — so the table never offers an ordering the source would refuse. Activating a header sorts by that column alone, cycling ascending, descending and back to the source's own order; Shift with a click, Enter or Space adds it as a further term. The headers show the ordering in force, the source's default included, and exactly one carries `aria-sort`. Without scripting each sortable header is a real link to the next ordering. Once scripts run, a menu beside each sortable header sorts it ascending or descending, or removes it from the ordering.
...
export function MachinesPage({ rows }: { rows: readonly Machine[] }) {
  ...
  return (
    <DataViews provider={provider}>
      <DataViews.SavedViews label="Machine views" />
      <DataViews.Search />
      <DataViews.Filters />
...
- **Your own words** — `<DataViews provider={provider} messages={{ search: "Rechercher", shownRows: ({ first, shown, total }) => … }}>`: the parts left out keep English, and a record typed `DataViewsMessages` replaces them all. Keep the record at module scope, or its functions stable: the root holds its words while their members stay the same.
...
- **Resizing and its limits** — held to the declared bounds; no control on the last column; two tables on one provider read the same presentation, so they always agree on every width, and nothing is passed between them.
