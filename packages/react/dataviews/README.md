...
React bindings for Canonical collection views, built on `@canonical/dataviews-core`: the `DataViews` root with its connected `Filters`, `Actions`, `Pagination` and `SavedViews` parts, the provider context, the four scoped observation hooks, the `DataTable` renderer and the `PaginationBar`.
