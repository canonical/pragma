 ...
 *
 * `Message` is fixed at wrap time — it is not reactive to the wrapped
 * component's props. When the message must follow prop state (e.g. a toggle
 * whose tooltip names the action for each state), render
 * {@link TooltipEngine} directly instead and pass `Message` as a live prop;
 * keeping one stable element type also keeps the target mounted across
 * state changes, preserving focus and hover state.
 *
