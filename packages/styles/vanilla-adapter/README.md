...
Guaranteed, and checked by the tests in `tests/` (`bun run test`): the binding test runs in node, the computed-style fixtures in a real Chromium through vitest's browser mode, against Vanilla 4.56 and 4.58 compiled at test time. Each line names its suite as it appears in the test output. One fixture waits for pragma's own reduced-motion rule and skips with the reason printed until it ships:

- The confined copy is pragma's element layers, rule for rule and declaration for declaration, under the mapping above; and `core.css` opens with pragma's statement, brings no element rule with it, and carries the scale and the registration. (`elements.css is pragma's element layers, confined`, `@canonical/styles exposes what a mixed page needs`, without a browser.)
...
- Under a reduced-motion preference nothing inside pragma territory animates, on the mixed page and on a pragma-only page alike. This needs pragma's own reduced-motion rule: Vanilla's `!important` in the lowest layer beats everything above it, so the two pages agree only once pragma disables its motion too. (`territory-equals-pragma-only`, skipped with that reason until then; today the non-guarantee below applies.)
- The document is pinned light, also under a dark operating system, and every pragma root computes its scheme from Vanilla's nearest theme ancestor: dark strip, light island inside dark, paper, nested root; a pragma theme class on a root inside a Vanilla page is ignored; and the token-driven colour of every theme case matches the light or the dark pragma page. (`theme-bridge`)
- After Vanilla is removed, the page behaves as rule 19 says: with this package still loaded every outermost island follows the operating system and pragma's theme class on a root stays ignored; with it swapped for `@canonical/styles` every root follows the theme classes. (`removal`)
...
- `layers.css` is one statement of the fourteen layers; `adapter.css` opens with its two imports and is the boundary block and the bridge block, and Chromium keeps the boundary's list whole; `elements.css` is the three layers with every rule confined; pragma's CSS uses no layer outside the statement and carries no `!important`; `@canonical/styles` and `core.css` open with pragma's own ten-name statement, the mixed order minus the adapter's four, so the later statement adds nothing and reorders nothing; and the order of `adapter.css` inside `pragma.css` does not matter. (`the order contract`, `order-independence`)
