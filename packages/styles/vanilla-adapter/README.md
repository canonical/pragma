...
Markup that breaks this renders with browser defaults rather than Vanilla's styles, which is the boundary doing its job. If you meet it, migrate that content first, or leave its container Vanilla until you can. (VC.03)
...
Until that release exists this package stays private. The computed-style fixtures that check its guarantees arrive with the second pull request, and it is published once they pass.
