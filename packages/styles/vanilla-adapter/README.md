...
- A pragma root placed directly in a Vanilla container that styles its children loses that styling, and a wrapper keeps it, as rule 8 says: an inline form's child spacing, a grid row's column placement, a Vanilla card's padding. (`territory-equals-pragma-only`, `root-not-styled`)
- Under a reduced-motion preference, pragma territory computes the same motion as on a pragma-only page. This needs pragma's own reduced-motion rule, not the scoped release: Vanilla's `!important` in the lowest layer beats everything above it, so the two pages agree only once pragma disables its motion too. (`territory-equals-pragma-only`, skipped with that reason until then; today the non-guarantee below applies.)
