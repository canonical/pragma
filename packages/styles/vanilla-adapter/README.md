...
- The confined copy is pragma's element layers, rule for rule and declaration for declaration, under the selector mapping in [DESIGN.md](./DESIGN.md). Two rules and one declaration exist only in the copy, each naming its reason in the test. (`tests/elements.test.ts`)
...
- An element that is itself an island root is styled as it would be on a pragma-only page, whatever element it is. (`tests/boundary.test.ts`, over thirty-one of them)
...
**The order statement has to be first, and nothing warns you if it is not.** A cascade layer takes its place in the order the first time a browser sees its name, and a later statement can add names but never move one. So if any stylesheet that mentions a `ds` layer reaches the browser before `layers.css` — a component package, or one of pragma's own entries — then `ds` is placed at the bottom of the order, below `boundary`, and the boundary's `all: revert` runs over pragma's own rules instead of Vanilla's. Every pragma component then renders as bare markup. There is no console warning and no partial failure; it is all or nothing. There is a second way in, with the same cause. A cascade layer's sublayers sort inside it, and a sublayer's position is fixed the first time its name appears too. So a stylesheet that opens a NAMED sublayer of `vanilla` before the statement is read — `@layer vanilla.mine { … }` — places that sublayer ahead of this package's own, and an `!important` declaration inside it then beats the second boundary. Measured: a list inside an island took Vanilla's `max-width: 640px` instead of the browser's `none`. Writing the same rule directly into `@layer vanilla { … }`, which is what installation step 2 asks for, is safe in either order.

Installation step 3 is the whole defence, `adapter.css` imports the statement itself so that the single-import path cannot go wrong, and step 7 is how you check. Measured in `tests/boundary.test.ts`, both ways.

**Seven pragma class names are not prefixed, and they match host markup.** `tokens.css` and `layout.css` are pragma's own entries and this package loads them whole, so their rules apply to any element on the page, not only inside an island. The names are `grid`, `subgrid`, `responsive`, `intrinsic` and `content-flow` from the layout presets, and `light` and `dark` from the theme modifiers. They sit in `ds.components.global` and `ds.modifiers`, both above `vanilla`, so where a host element already carries one of these names pragma wins. `grid` is the one worth checking for. Rename yours. Dropping the entry is not an option this package offers: `adapter.css` imports `layout.css`, and `light` and `dark` arrive with `tokens.css` through the design tokens, so avoiding them means hand-resolving `adapter.css` rather than importing it. The collision is pragma's to fix upstream, and this package cannot confine it without shipping a second copy.

**Answering a Vanilla `!important` costs you that property inside four utilities.** An important declaration can only be answered by another one from a lower layer, so the four counters this package writes are important too. Wherever one fires it outranks *your* value for that property as well as Vanilla's. That is inside `.u-text-max-width` for `max-width` on a list, inside `.u-table-layout--fixed` or `--auto` for `table-layout` on a table, inside `.p-content-card__author-and-date` for `margin-bottom` on a first child that is not a heading or a paragraph, and inside `.u-vertically-center` for `align-self` on an image. Each fires only under that Vanilla class, so moving the island out of the utility, or setting the property on an inner element instead of the island root, gets it back.

**One Vanilla utility cannot be answered at all.** `.u-table-layout--auto` forces `table-layout: auto`, which is the browser's own default. A counter can only revert, and reverting lands on that same value, so a pragma table inside that utility cannot get back a `table-layout: fixed` it asked for. Measured: `auto` with a counter and `auto` without one, against the `fixed` a pragma-only page computes. There is no counter for it, because one that changes nothing would read as cover. Move the table out of the utility, or set the layout on an inner table.

**Three typographic properties are stopped for everyone, not only for Vanilla.** The island root declares `word-spacing`, `text-shadow` and `text-underline-offset`, so a host page's own value for them does not reach the island either. Nothing in CSS can tell a host's deliberate choice from a framework's, and these three are the ones where Vanilla itself would never show the difference. Set them inside the island if you want them.

**An island root does not inherit a table cell's text alignment or its boldness.** The browser centres a `<th>`, and on a pragma-only page an island *inside* one inherits that. Here the island root declares `text-align: start` and `font-weight: normal`, because those same properties are how Vanilla's `u-align--*` family and its heading classes would otherwise reach in, and nothing in CSS can tell a browser default from a framework's rule. The trade buys off the larger leak. Set either on the island yourself where you want it.
