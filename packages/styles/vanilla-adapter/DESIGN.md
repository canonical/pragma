...
@layer boundary {
  @scope (.ds) to (:scope.ds-permeable > *) {
...
`:scope.ds-permeable` only matches the scope root, and only when that root itself carries `ds-permeable`; an ordinary island, `.ds` alone, never matches it, so the limit never fires and the block behaves exactly as it did before this existed. Where it does match, `> *` makes the limit every one of the root's own children, and a scope's lower bound is exclusive: a matched limit and everything below it fall outside the scope. Nothing below the root is confined or reverted, while the root itself still is, because a scope's upper bound is always inclusive regardless of what the limit matches.
...
- One declaration and two rules exist only in the copy. The declaration is `font-size: 1rem` on the island root: a document element has nothing above it to inherit a size from and a size declared there would override the reader's own, which is why pragma's reset leaves it out; an island root has a Vanilla ancestor that can size its text. The two rules give `small`, `sub` and `sup` the same proportion of the root, because the sizes `normalize` gives them are percentages of their parent, which on a mixed page is the host's. `tests/elements.test.ts` names the declaration in `COPY_ADDS` and the rules in `COPY_ONLY`, each with its reason, so an exception has to be stated to pass.
- One selector shape has no original: `:where(:scope:not(.ds *)):is(…)`, the island root narrowed to the elements the browser gives a value RELATIVE to their parent. Those cannot simply be excluded, because excluding them hands them to the host.
...
Seven of the root's rules carry an exclusion list on top of that, because the value they set is one the browser already gets right on some elements and an island root can be any element. `font-family` and `line-height` exclude the elements `normalize` sizes itself; `color`, `font-weight`, `font-style`, `text-align` and `white-space` exclude the ones the browser greys, bolds, italicises, centres or keeps preformatted. `font-size` carries no list, because every size `normalize` gives is relative to the parent. On a document element, which is never a `<pre>` or a `<th>`, every one of those lists is inert, which is why they live in pragma's own files and ride across mechanically:
