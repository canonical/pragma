...
- **Query grammar** — `Slice` (filters, search, ordered sort terms, ordered grouping levels) and `ResultWindow` (page, size, cursor token, collapsed group paths), paired as `Query`; semantic equality is the provider's own — `isAny` and `isNone` operands are sets, sort order is never reordered, empty search is no search — and `EMPTY_SLICE` and `DEFAULT_WINDOW` — the query that restricts nothing and the window a collection starts on — as the two halves of a `Query` to encode — a link to the first page of failed machines is `encodeQuery({ schema: machines.schema, slice: { ...EMPTY_SLICE, filter: [{ field: "status", operator: "isAny", operands: ["failed"] }] }, window: DEFAULT_WINDOW })`.
- **Schema** — `collection.schema`, built by `createSchema` from the collection's field list: the one coherent construction path; field kinds (`choices`, `number`, `flag`, `date`, `text`) decide legal operators, how an ordered term over the field compares, and inferred applied types; inputs and direct operands are validated against option membership, numeric range and ISO-8601 dates. A `choices` field is filtered by `isAny` — its value is one of the operands — and `isNone` — its value is present, a string or a number, and none of them; an absent or null value is neither. A `text` field is ordered, and filtered by `contains` and `startsWith`: its value holds the operand, or begins with it, once both are folded — normalised to NFC, lowercased without a locale, final sigma read as sigma, and normalised to NFC again. The operand is taken literally, and `schema.listOperators("name")` is `["contains", "startsWith"]`.
...
- **Wire grammar** — `encodeQuery`/`decodeQuery`: the flat, form-compatible spelling of a query as URL parameters (`status=failed&status=cancelled`, `status__isNone=ready`, `cpu__gte=4`, `owner__isSet=1`, `name__contains=web`, `name__startsWith=api`, `q=yak`, `sort=updated__desc`, `group=status`, `page=2&size=50&cursor=…`). `isAny` is written as the bare field name; `status__isAny=failed` is read as the same predicate, its operands joined to any the bare spelling carries, and written back bare. Values are parsed to their field's own type, operators are checked against the field's kind, and — given the source's capabilities — clauses the source cannot execute are refused too; every refused clause is left out of the query and reported by parameter. Parameters naming no field of the collection are the host's and survive untouched. A host builds its own links, such as no-JS pagination destinations, with `encodeQuery` and the parameters it wants preserved. Given a null window, `encodeQuery` writes the query alone — the text a saved view stores.
...
- **Writing a source** — `declareCapabilities(collection, declaration)` builds the complete frozen capability record a source publishes, typed against the collection's schema: a filter operator is checked per field kind, so `status: ["gte"]` on a `choices` field does not compile, and a member left out is refused — no filter on that field, no search, no sort, counts unknown, no actions, no facets. A facet is what a source computes over every record the query matches, with that field's own predicates lifted so the other values keep their counts: for a `choices` or `flag` field each value held with its count, for a `number` or `date` field the least and greatest value. A source declares the fields it computes facets for, the provider's `facets` names the ones every request asks for — throwing for one the source does not declare, and for a filterable `choices` field whose options are the server's and whose facet it leaves out — and the page answers them, on `result.facets`. `readSlice(collection, slice)` reads a request's slice by field, typed from the schema — a field whose kind accepts several operators as a record keyed by operator: a `choices` filter as its `isAny` and `isNone` sets, a number or date filter as its bounds, a text filter as the text it `contains` and `startsWith`; a flag as `true` — plus the search text and the ordered sort terms — so an adapter finds the status filter by name instead of scanning predicates. `createPage({ rows, matched, total, more, cursors, facets })` builds the envelope: a count given is exact, a count left out is unknown, and the pageable count is the matched count while nothing collapses rows out of a page.
