...
- **Identity tokens** — `createIdentity`, `isIdentity` (from `./bindings`): referential scope markers; structural copies and forged-key look-alikes are rejected. The provider's own token is `provider.identity`.
- **Query grammar** — `Slice` (filters, search, ordered sort terms, ordered grouping levels) and `ResultWindow` (page, size, cursor token, collapsed group paths), paired as `Query`; `canonicalSlice` (equality operands are sets, sort order is never reordered, empty search is no search) and `sliceEquals` for semantic equality, both from `./bindings`, and `EMPTY_SLICE` and `DEFAULT_WINDOW` — the query that restricts nothing and the window a collection starts on — as the two halves of a `Query` to seed, adopt or encode.
...
- **Observation** — every record publishes its state on a `ReadonlyChannel`, the read side handed to projections, with equality-guarded notification. `provider.selection` holds explicit record identities with a valid empty state. Framework bindings mint channels of their own with `createChannel` from `./bindings`.
