...
Under active development; both DataViews packages are unpublished, so the surface changes without compatibility. The root exports what an application calls — `createSchema`, `createDataViewsProvider`, the sources `createArraySource`, `createQuerySource` and `createRelaySource` with `createSourceBinding`, the adapter helpers `declareCapabilities`, `readSlice` and `createPage`, the locations `createMemoryLocation` and `createPlatformLocation` with `createLocationBinding`, the wire grammar `encodeQuery` and `decodeQuery`, and the two seed constants `EMPTY_SLICE` and `DEFAULT_WINDOW` — together with the grammar, schema, result, refusal, count, capability, provider and saved-view types anyone writing a source or reading a result must name (`Slice`, `ResultWindow`, `Query`, `SourcePage`, `Completion`, `SourceCapabilities`, `ViewStore`, `Schema`, `Selection` and friends). What the provider builds and owns — the coordinator, the field interaction records, the operation record, the row model, the selection and the channels — is not exported. Three further entry points carry bytes or an audience the root does not:

- `@canonical/dataviews-core/bindings` — the machinery every framework binding shares and no application calls: the column geometry (`resolveColumns`, `buildColumnTemplate`, `createColumnLayout`, `createGridInteraction`, `areSizingsEqual`), the row scopes and display entries a body renders from (`createRowScopes`, `listDisplayEntries`), the identity check (`isIdentity`) and the list and slice comparisons (`areListsEqual`, `areSlicesEqual`).
...
- **Identity tokens** — referential scope markers; structural copies and forged-key look-alikes are rejected by `isIdentity` (from `./bindings`). The provider's own token is `provider.identity`.
...
- **Observation** — every record publishes its state on a `ReadonlyChannel`, the read side handed to projections, with equality-guarded notification. `provider.selection` holds explicit record identities with a valid empty state.
