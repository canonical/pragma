...
- **Schema** — `createSchema`: the one coherent construction path; field kinds (`choices`, `number`, `flag`, `date`) decide legal operators and inferred applied types; inputs and direct operands are validated against option membership, numeric range and ISO-8601 dates.
