...
const harness = (
  ...
  const publish = (next: readonly Machine[]): void => {
    channel.set(createRowModel(next, undefined, channel.get()));
  };
...
describe("createRowScopes", () => {
  ...
  it("re-reads a row's fields only when its record changes", () => {
    ...
    const channel = createChannel(createRowModel([unchanged]));
    const selection = createSelection();
    ...
    channel.set(createRowModel([unchanged], undefined, channel.get()));
    expect(reads).toBe(minted);
    // A replaced record is read once per observed field.
    channel.set(createRowModel([counted("stopped")], undefined, channel.get()));
  ...
  it("observes nothing until it is asked to, then catches up", () => {
    ...
    channel.set(createRowModel([machine("m-1"), machine("m-2")]));
    selection.set(["m-1"]);
    expect(scopes.ids.get()).toEqual(["m-1"]);
    scopes.observe();
    expect(scopes.ids.get()).toEqual(["m-1", "m-2"]);
