...
import type { Collection, DiscriminatorField } from "./types.js";
...
describe("createCollection types", () => {
  it("infers the record type from the identity function", () => {
    const collection = createCollection({
      identify: (row: Container) => row.id,
      fields: [{ field: "type", kind: "choices", options: ["container"] }],
    });
    const typed: Collection<typeof collection.schema.fields, Container> =
      collection;
    // @ts-expect-error a collection over containers is not one over machines
    const other: Collection<typeof collection.schema.fields, VirtualMachine> =
      collection;
    expect([typed, other]).toHaveLength(2);
  });

  it("refuses a discriminator the record type cannot carry", () => {
    expect(() =>
      createCollection({
