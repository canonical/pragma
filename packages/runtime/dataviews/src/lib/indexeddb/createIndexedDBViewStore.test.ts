...
import type { JsonValue, PresentationStore } from "../presentation/index.js";
import type { SavedView, ViewStore } from "../views/index.js";
import createIndexedDBViewStore from "./createIndexedDBViewStore.js";
import type { IndexedDBFactory, IndexedDBViewStoreConfig } from "./types.js";
...
const opened: (ViewStore & PresentationStore)[] = [];
...
const createRefusingFactory = (): IndexedDBFactory => ({
  open() {
    throw new DOMException("access to storage is denied", "SecurityError");
  },
});

describe("createIndexedDBViewStore", () => {
  describe("saved views", () => {
    it("creates a view and reads it back", async () => {
      ...
      const view = readSaved(await store.create(failed));
      expect(view).toEqual({
        id: "failed",
        name: "Failed",
        query: "status=failed",
    ...
    it("finds the view a lost creation made when it is retried", async () => {
      const store = openStoreOver(new IDBFactory());
      const first = readSaved(await store.create(failed));
    ...
    it("refuses a change made against a stale revision", async () => {
      ...
      expect(
        readSaved(await store.update(current, { name: "Two" })),
      ).toMatchObject({
        name: "Two",
        revision: 3,
      });
  ...
  describe("viewer preferences", () => {
    it("locks the preferences alone to read them, or to change the default arrangement", async () => {
      const indexedDB = new IDBFactory();
      const store = openStoreOver(indexedDB);
      const transaction = vi.spyOn(IDBDatabase.prototype, "transaction");
      await store.readPresentation("default");
      await store.patchPresentation("default", { width: 1 });
      // A view's own preferences are written only while the view exists.
      await store.patchPresentation({ view: "v1" }, { width: 1 });
      // The upgrade's own transaction aside.
      expect(
        transaction.mock.calls
          .map((call: readonly unknown[]) => call.slice(0, 2))
          .filter((call: readonly unknown[]) => {
            const names = call.at(0);
            return Array.isArray(names) && names.length > 0;
          }),
      ).toEqual([
        [["preferences"], "readonly"],
        [["preferences"], "readwrite"],
        [["views", "preferences"], "readwrite"],
      ]);
    });
    ...
    it("leaves a pin alone when a change is saved", async () => {
      ...
      expect(
        readSaved(
          await store.update({ id: "failed", revision: 1 }, { name: "x" }),
        ).pinned,
      ).toBe(true);
    });
    ...
    it("pins nothing for a view that does not exist", async () => {
      ...
      expect(await store.get("gone")).toMatchObject({
        view: { pinned: false },
      });
    });
    ...
    it("removes a view with its pin and its own preferences only", async () => {
      ...
      expect(await store.get("ready")).toMatchObject({
        view: { pinned: true },
      });
    });
    ...
    it("reads a stored preference only when it has the shape of one", async () => {
      ...
      await store.create(failed);
      await store.patchPresentation("default", { density: "compact" });
      const buildPreference = (fields: Record<string, unknown>) => ({
        scope: machinesScope,
        target: "default",
        ...fields,
      });
      await writeRaw(
        ...
        [
          ...
          buildPreference({ key: "width", value: undefined }),
          buildPreference({ key: "zoom", value: Number.POSITIVE_INFINITY }),
          buildPreference({ key: "order", value: [1, new Date(0)] }),
          buildPreference({ key: "layout", value: { nested: new Map() } }),
          buildPreference({ key: 7, value: true }),
          buildPreference({
            key: "columns",
            value: { visible: ["a", null], n: 1 },
          }),
          // A pin whose key is not a view id pins nothing.
          buildPreference({ target: "pins", key: 7, value: true }),
          buildPreference({ target: "pins", key: "failed", value: "yes" }),
        ],
        "preferences",
      );
  ...
  describe("across tabs", () => {
    it("makes two tabs editing one view conflict instead of clobbering", async () => {
      const indexedDB = new IDBFactory();
      const first = openStoreOver(indexedDB);
      const second = openStoreOver(indexedDB);
    ...
    it("tells another tab's listeners so they read again", async () => {
      ...
      expect((await second.list()).views.map(({ id }) => id)).toEqual([
        "failed",
      ]);
    });
    ...
    it("tells a listener subscribed after dispose nothing where the platform has no BroadcastChannel", async () => {
      ...
      expect(readSaved(await creating).id).toBe("failed");
    ...
    it("closes for another tab's upgrade instead of blocking it", async () => {
      ...
      await expect(listing).rejects.toHaveProperty(
        "cause.name",
        "VersionError",
      );
    });
  ...
  describe("scope and lifecycle", () => {
    it("never shows one partition another's views", async () => {
      const indexedDB = new IDBFactory();
      const alice = openStoreOver(indexedDB, { partition: "account:alice" });
      const bob = openStoreOver(indexedDB, { partition: "account:bob" });
      const heard = vi.fn();
      bob.subscribe(heard);
      // A second tab of Alice's hears her writes, so Bob has had his chance.
      const aliceElsewhere = vi.fn();
      openStoreOver(indexedDB, { partition: "account:alice" }).subscribe(
        aliceElsewhere,
      );
    ...
    it("never shows one collection another's views", async () => {
      const indexedDB = new IDBFactory();
      await openStoreOver(indexedDB).create(failed);
      expect(
        await openStoreOver(indexedDB, { collection: "clusters" }).list(),
    ...
    it("detaches on dispose: calls reject and listeners are dropped", async () => {
      ...
      const witness = vi.fn();
      openStoreOver(indexedDB).subscribe(witness);
      await openStoreOver(indexedDB).pin("failed");
    ...
    it("never lets a throwing listener fail a committed write", async () => {
      ...
      expect(readSaved(await store.create(failed)).id).toBe("failed");
    ...
    it("tells other tabs of a write that committed after it was disposed", async () => {
      const indexedDB = new IDBFactory();
      const leaving = openStoreOver(indexedDB);
      const staying = openStoreOver(indexedDB);
      const heard = vi.fn();
    ...
    it("lets a failing open and a late platform close pass after dispose", async () => {
      const indexedDB = new IDBFactory();
      await upgradeElsewhere(indexedDB);
      const failing = openStoreOver(indexedDB);
  ...
  describe("failures", () => {
    it("rejects when storage is unavailable and never claims to have saved", async () => {
      const store = openStoreOver(createRefusingFactory());
      const creating = store.create(failed);
     ...
     */
    const openConnectingTo = (transaction: () => unknown): IndexedDBFactory => {
      const request = {
        result: {
          transaction,
          close() {},
    ...
    it("rejects a commit the platform aborts for want of room", async () => {
      ...
      const objectStore = {
        get: buildPendingRequest,
        put: buildPendingRequest,
        delete: buildPendingRequest,
        index: () => ({
          getAll: buildPendingRequest,
          getAllKeys: buildPendingRequest,
        }),
      };
      const store = openStoreOver(
        openConnectingTo(() => {
  ...
  describe("stored records", () => {
    const buildRecord = (fields: Record<string, unknown>) => ({
      scope: machinesScope,
      v: 1,
      name: "Stored",
      query: "status=failed",
    ...
    it("reports records it cannot read and never overwrites them", async () => {
      ...
      const store = openStoreOver(indexedDB);
      await store.list();
      const newer = buildRecord({ id: "newer", v: 2, owner: "team:ops" });
      await writeRaw(indexedDB, [
        buildRecord({ id: "older", v: undefined }),
        newer,
        buildRecord({ id: "renamed", name: 7 }),
        buildRecord({ id: "listed", presentation: ["not", "keyed"] }),
