...
## Annotation-driven binding: the `graphql:` vocabulary
...
lib:Book     graphql:name "Publication" ;         # verbatim type name
             ...
             graphql:titleFrom lib:heading .      # _meta.title reads lib:heading first
...
> Declare the prefix exactly as above. The extractor matches annotation terms by namespace: assertions under any other IRI — including near-misses like an `http://` form — are ignored without a diagnostic.
...
| `V001–V016` | data/ontology validation | blank-node-only class (`V001`), domainless property (`V002`), boolean-as-string (`V006`), SHACL `sh:maxCount 0` omission (`V010`), undeclared ABox predicate (`V014`), class forced abstract by `graphql:abstract` yet carrying direct instances (`V015`), concrete supertype flattening (`V016`) |
