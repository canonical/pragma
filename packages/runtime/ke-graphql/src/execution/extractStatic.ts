...
import type { CompilerContext, MappedIR } from "../compiler/types.js";
...
/** All named instance URIs (prefixed) across all concrete classes. */
const allEntityUris = async (
  mapped: MappedIR,
  context: CompilerContext,
): Promise<string[]> => {
  const uris: string[] = [];
...
export const extractStatic = async (
  ...
  for (const query of options.queries) {
    ...
    if (
      ...
      entityUris ??= await allEntityUris(options.mapped, options.context);
