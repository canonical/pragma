...
export default function createGraphQLHandler(
  ...
  return async (request: Request): Promise<Response> => {
    ...
    if (request.method === "GET") {
      ...
      // `null` (and JSON primitives) parse without throwing but carry no query;
      // normalize so a one-byte `null` body yields a clean 400, not a TypeError.
      if (typeof body !== "object" || body === null) {
        body = {} as GraphQLRequestBody;
      }
    } else {
  ...
  function buildMultipartResponse(
    ...
    // Run every streamed payload's errors through the same masking/formatting
    // as the non-streaming paths — masked internal errors must not leak on the
    // multipart path either (top-level errors, and per-entry errors in the
    // graphql-v17 incremental/completed arrays).
    type ErrList = ReadonlyArray<GraphQLError | GraphQLFormattedError>;
    const maskList = (errors: ErrList): GraphQLFormattedError[] =>
      errors.map(formatExecutionError);
    const maskEntries = (entries: ReadonlyArray<Record<string, unknown>>) =>
      entries.map((entry) =>
        entry.errors
          ? { ...entry, errors: maskList(entry.errors as ErrList) }
          : entry,
      );
    // Streamed payloads always carry errors/incremental/completed as arrays
    // when present (graphql-v17 / Relay payload shape).
    const maskPayload = (payload: unknown): unknown => {
      const p = payload as Record<string, unknown>;
      return {
        ...p,
        ...(p.errors ? { errors: maskList(p.errors as ErrList) } : {}),
        ...(p.incremental
          ? {
              incremental: maskEntries(
                p.incremental as Record<string, unknown>[],
              ),
            }
          : {}),
        ...(p.completed
          ? { completed: maskEntries(p.completed as Record<string, unknown>[]) }
          : {}),
      };
    };
        ...
        try {
          if (format === "relay-legacy") {
            for await (const payload of relayFormatAdapter(results)) {
              controller.enqueue(encodePart(maskPayload(payload)));
            }
          } else {
            controller.enqueue(encodePart(maskPayload(results.initialResult)));
