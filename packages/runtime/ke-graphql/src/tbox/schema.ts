...
//
// Structural facts come from the frozen IR (resolver closures); only
// annotations (acceptanceCriteria/completionGuidance) hit the tboxLoader.
// =============================================================================
...
export const buildTBoxSchema = (
  ...
  const cardinalityFor = (property: PropertyNode, classUri: string) => {
    ...
    fields: () => ({
      ...
      required: {
        ...
        resolve: (cp) => {
          ...
          return property
            ? cardinalityFor(property, cp.classUri).required
            : false;
        },
      },
      singular: {
        ...
        resolve: (cp) => {
          const property = ir.properties.get(cp.propertyUri);
          return property
            ? cardinalityFor(property, cp.classUri).singular
            : false;
        },
      },
      inherited: {
  ...
  const classPropertiesFor = (node: ClassNode): ClassPropertyValue[] =>
    node.allProperties
  ...
  >({
    ...
    fields: () => ({
      ...
      properties: {
        ...
        resolve: (c) => classPropertiesFor(c),
      },
      instances: {
    ...
    fields: () => ({
      ...
      fields: {
        ...
        resolve: (parent) => {
          ...
          return node ? classPropertiesFor(node) : [];
        },
      },
    }),
  });
