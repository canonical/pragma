...
/** Run a SELECT and return its term-preserving rows, or [] + diagnostic. */
const select = async (
  query: QueryFn,
  sparqlText: string,
...
const named = (term: Term | undefined): string | undefined =>
  term?.termType === "NamedNode" ? term.value : undefined;

const literal = (term: Term | undefined): string | undefined =>
  term?.termType === "Literal" ? term.value : undefined;

const int = (term: Term | undefined): number | undefined => {
  const value = literal(term);
  if (value === undefined) {
    return undefined;
  }
...
/**
 * Extract the TBox structure and ABox probes from a ke store.
 */
export const extract = async (
  ...
  for (const row of classRows) {
    const uri = named(row.class);
    if (!uri || isStandardVocab(uri)) {
      continue;
    }
    const entry = classMap.get(uri) ?? { uri, superclasses: [] };
    entry.label ??= literal(row.label);
    entry.definition ??= literal(row.definition) ?? literal(row.comment);
    const superclass = named(row.superclass);
  ...
  const kindOf = (kindUri: string | undefined): RawPropertyKind => {
  ...
  for (const row of propertyRows) {
    const uri = named(row.prop);
    if (!uri || isStandardVocab(uri)) {
      continue;
    }
    const entry = propertyMap.get(uri) ?? {
      uri,
      kind: kindOf(named(row.kind)),
      domains: [],
      ranges: [],
    };
    entry.label ??= literal(row.label);
    entry.definition ??= literal(row.definition) ?? literal(row.comment);
    const domain = named(row.domain);
    if (domain && !entry.domains.includes(domain)) {
      entry.domains.push(domain);
    }
    const range = named(row.range);
  ...
  const inverses = inverseRows.flatMap((row) => {
    const property = named(row.prop);
    const inverse = named(row.inverse);
  ...
  const functionals = new Set(
    functionalRows.flatMap((row) => named(row.prop) ?? []),
  );
  ...
  for (const row of datatypeRows) {
    const uri = named(row.dt);
    if (!uri || isStandardVocab(uri)) {
      continue;
    }
    const entry = datatypeMap.get(uri) ?? { uri };
    entry.baseType ??= named(row.base);
    entry.pattern ??= literal(row.pattern);
  ...
  for (const uri of [...classMap.keys(), ...propertyMap.keys()]) {
    discovered.add(namespaceOf(uri));
  }
  let anonymous = 0;
  ...
  for (const row of inListRows) {
    const path = named(row.path);
    const value = literal(row.value) ?? named(row.value);
  ...
  for (const row of directShaclRows) {
    const targetClass = named(row.targetClass);
    const path = named(row.path);
    if (!targetClass || !path) {
      continue;
    }
  ...
  for (const row of anonUnionRows) {
    const property = named(row.property);
    const member = named(row.member);
    if (!property || !member) {
      continue;
    }
  ...
  for (const row of statsRows) {
    ...
    instanceStats.set(uri, { total, named: int(row.named) ?? 0 });
  }
  ...
  const selfReferential = new Set(
    selfRows.flatMap((row) => named(row.p) ?? []),
  );
  ...
  for (const row of predicateRows) {
    ...
    if (namespaces.has(namespaceOf(uri))) {
