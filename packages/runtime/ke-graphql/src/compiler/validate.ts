  ...
  for (const node of ir.classes.values()) {
    ...
    if (isBlankOnly(ir, node.uri)) {
      push({
        severity: "warning",
        code: "V001",
        message: `${localName(node.uri)} instances are exclusively blank nodes — embeddable type`,
        source: node.uri,
      });
    }
    ...
    for (const parent of node.superclasses) {
      if (!ir.classes.has(parent) && isStandardVocab(parent)) {
        push({
          severity: "warning",
          code: "V009",
          message: `${localName(node.uri)} subClassOf standard-vocabulary ${parent} — treated as root class`,
          source: node.uri,
        });
        ...
        push({
          severity: "warning",
          code: "B002",
          message: `${localName(node.uri)} references unknown superclass ${parent}`,
          source: node.uri,
        });
      }
    }
  ...
  for (const property of ir.properties.values()) {
    ...
    for (const domain of property.domains) {
      if (!ir.classes.has(domain) && !isStandardVocab(domain)) {
        push({
          severity: "warning",
          code: "B002",
          message: `${localName(property.uri)} has unknown domain ${domain}`,
          source: property.uri,
        });
      }
    }
    ...
    if (property.range.kind === "unknown") {
      push({
        severity: "warning",
        code: "B003",
        message: `${localName(property.uri)} has unknown range ${property.range.raw} — mapped to String`,
        source: property.uri,
      });
    }
    ...
    if (property.inverse && !ir.properties.has(property.inverse)) {
      push({
        severity: "warning",
        code: "B004",
        message: `${localName(property.uri)} declares unknown inverse ${property.inverse}`,
        source: property.uri,
      });
    }
    // V002 — domainless
    if (property.domains.length === 0 && !property.isAnnotation) {
      push({
        severity: "warning",
        code: "V002",
        message: `${localName(property.uri)} has no rdfs:domain — assigned to all ${property.namespace}: classes`,
        source: property.uri,
      });
    }
    ...
    if (property.inverse) {
      ...
      if (other && other.inverse !== property.uri) {
        push({
          severity: "warning",
          code: "V003",
          message: `asymmetric owl:inverseOf between ${localName(property.uri)} and ${localName(property.inverse)}`,
          source: property.uri,
        });
      }
    }
    ...
    if (ir.extraction.selfReferential.has(property.uri)) {
      push({
        severity: "warning",
        code: "V004",
        message: `${localName(property.uri)} has a self-referential assertion in the data`,
        source: property.uri,
      });
    }
    ...
    if (ir.extraction.functionalViolations.has(property.uri)) {
      push({
        severity: "warning",
        code: "V005",
        message: `functional property ${localName(property.uri)} has multiple values on some instance`,
        source: property.uri,
      });
    }
    ...
    if (
      ...
      push({
        severity: "info",
        code: "V006",
        message: `${localName(property.uri)} is xsd:boolean — resolver coerces string literals`,
        source: property.uri,
      });
    }
    ...
    if (property.isAnnotation) {
      push({
        severity: "info",
        code: "V007",
        message: `annotation property ${localName(property.uri)} routed to TBox schema`,
        source: property.uri,
      });
    }
    ...
    if (property.range.kind === "scalar" && property.range.customDatatype) {
      push({
        severity: "info",
        code: "V008",
        message: `custom datatype ${localName(property.range.customDatatype)} mapped to ${property.range.graphqlScalar}`,
        source: property.uri,
      });
    }
    ...
    for (const [classUri, spec] of property.classCardinality) {
      if (spec.omit) {
        push({
          severity: "warning",
          code: "V010",
          message: `${localName(property.uri)} has sh:maxCount 0 on ${localName(classUri)} — field omitted`,
          source: property.uri,
        });
      }
    }
    ...
    if (fromOr) {
      push({
        severity: "info",
        code: "V011",
        message: `${localName(property.uri)} constrained via sh:or — most permissive interpretation applied`,
        source: property.uri,
      });
    }
    ...
    if (inConstraint) {
      push({
        severity: "info",
        code: "V012",
        message: `${localName(property.uri)} has sh:in (${inConstraint.inValues?.join(", ")}) — mapped to String, enum deferred`,
        source: property.uri,
      });
    }
    // V013 — multi-domain
    if (property.domains.length > 1) {
      push({
        severity: "warning",
        code: "V013",
        message: `${localName(property.uri)} declares ${property.domains.length} domains — OWL means intersection; field generated on each`,
        source: property.uri,
      });
    }
  }
