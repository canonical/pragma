...
import { isStandardVocab, localName, namespaceOf, XSD } from "./vocab.js";

const PHASE = "build";

const XSD_SCALARS: Record<string, "String" | "Boolean" | "Int" | "Float"> = {
  [`${XSD}string`]: "String",
  [`${XSD}boolean`]: "Boolean",
  [`${XSD}integer`]: "Int",
  [`${XSD}int`]: "Int",
...
export const build = (
  ...
  const prefixOf = (uri: string): string =>
    extraction.namespaces.get(namespaceOf(uri)) ?? "";
  ...
  const mappingFor = (uri: string) => {
    const direct = mappings[uri];
    if (direct) {
      return direct;
    }
    const prefix = prefixOf(uri);
    return prefix ? mappings[`${prefix}:${localName(uri)}`] : undefined;
  };
  ...
  for (const raw of extraction.classes) {
    ...
    classes.set(raw.uri, {
      uri: raw.uri,
      label: raw.label ?? localName(raw.uri),
      definition: raw.definition,
      namespace: prefixOf(raw.uri),
  ...
  const computeAncestors = (uri: string, trail: string[]): string[] => {
    ...
    if (trail.includes(uri)) {
      diagnostics.push({
        severity: "error",
        code: "B001",
        message: `subClassOf cycle: ${[...trail, uri].map(localName).join(" → ")}`,
        source: uri,
        phase: PHASE,
      });
      return [];
  ...
  for (const node of classes.values()) {
    ...
    const mapping = mappingFor(node.uri);
    const isAbstract =
  ...
  for (const raw of extraction.properties) {
    ...
    const mapping = mappingFor(raw.uri);
    const functional =
      mapping?.singular ??
