...
import { localName } from "./vocab.js";
...
const RESERVED_FIELD_NAMES = new Set(["id", "uri", "_meta", "__typename"]);
...
const mappingFor = (
  ...
  return ns ? state.options.mappings?.[`${ns}:${localName(uri)}`] : undefined;
};

/** Resolve a class URI to its GraphQL type name with collision handling. */
...
const fieldNameFor = (
  ...
  const custom = mappingFor(state, property.uri)?.graphqlName;
  if (custom) {
    return custom;
  }
  let name = stripVerbPrefix(localName(property.uri));
  if (isList) {
    name = pluralize(name);
  }
...
const fieldTypeFor = (
  ...
  switch (property.range.kind) {
    ...
    case "union": {
      const name =
        property.range.name ??
        `${localName(property.uri).charAt(0).toUpperCase()}${localName(property.uri).slice(1)}Union`;
      ...
      for (const member of property.range.members) {
        ...
        if (node.isAbstract) {
          for (const sub of concreteDescendants(state.ir, node)) {
...
const fieldsFor = (
  ...
  for (const propertyUri of node.allProperties) {
    ...
    const list = !singular;
    const name = fieldNameFor(state, property, list);
    const nonNull =
  ...
  for (const property of state.ir.properties.values()) {
    ...
    const synthetic = mappingFor(state, property.uri)?.inverse;
 ...
 */
const interfaceIsEmbeddable = (ir: OntologyIR, node: ClassNode): boolean =>
  concreteDescendants(ir, node).every(
...
export const map = (
  ...
  for (const key of Object.keys(options.mappings ?? {})) {
    ...
    const prefixed = [...ir.classes.values(), ...ir.properties.values()].some(
      (n) => `${n.namespace}:${localName(n.uri)}` === key,
    );
    if (!full && !prefixed) {
      diagnostics.push({
        severity: "warning",
  ...
  for (const node of ir.classes.values()) {
    ...
    const implemented = node.ancestors
      ...
      .filter((a) => !node.embeddable || interfaceIsEmbeddable(ir, a))
    ...
    types.set(typeName, {
      ...
      singularName: camelCase(typeName),
      pluralName: pluralize(camelCase(typeName)),
    });
  }
