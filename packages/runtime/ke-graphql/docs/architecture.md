...
|---|---|---|---|
| 1 Extract | `extract.ts` | store → `RawExtraction` | 17 SPARQL queries: **11 TBox** (classes, properties, inverses, functionals, custom datatypes, SHACL direct + `sh:in` values + `sh:or` branches, named + anonymous unions, every `graphql:`-namespace assertion — deduplicated, sorted, both value kinds with their kind, mode-independent so one artifact serves any mode at rebuild) and **6 ABox probes** (instance counts, self-reference, functional violations, undeclared predicates, annotation assertions, blank-node nesting depth). Two are conditional — the functional-violation probe needs a declared `owl:FunctionalProperty`, the annotation probe a declared annotation property. The file's `Q1…Q12` markers number the extraction *steps*, not the queries (`Q6`, namespace discovery, runs in code and issues none — it resolves registered prefixes over synthetics and records the namespaces whose synthetic-prefix warning it cannot yet decide; a `graphql:prefix` declaration binds in Pass 2, where the mode is known; `Q7a` and `Q8` issue two each). All store access happens here so passes 2–7 stay pure. |
| 2 Build | `build.ts` | `RawExtraction` → `OntologyIR` | resolves the `graphql:` annotation overlay first (`annotations.ts` — the A-band diagnostics; skipped under `mode: "auto"`, A006), then the effective namespace → prefix map (Pass 1's map with the validated `graphql:prefix` declarations folded over it) and the deferred `E001` warnings it settles, subClassOf closure, abstract/embeddable detection (annotation > instance-stats heuristic), per-class cardinality by precedence (`graphql:singular` > `owl:FunctionalProperty` > `owl:cardinality` > SHACL > kind default), annotation inverse pairs joining `owl:inverseOf`, range resolution. The overlay rides the IR as `OntologyIR.graphql`. |
...
| 4 Map | `map.ts` | `OntologyIR` → `MappedIR` | GraphQL naming rules (`graphql:name` joins the custom-name channel), collision auto-resolution (namespace prefixing), the `mode: "explicit"` expose allowlist (`A007`/`A008`), `graphql:nonNull` promotion, declared-inverse field wiring, the bidirectional name map. |
...
- **Inverse pairs.** `owl:inverseOf` produces one field per side, never the four-way duplication. At resolution each side takes the **union of forward and reverse assertions**, so data asserted in either direction answers identically from both ends. `graphql:inverse` declares a pair with the same semantics. Only DECLARED pairs produce inverse fields: a one-directional property with no partner projects a field on its domain class alone, and Pass 4 mints nothing on the far side.
...
The compiler collects problems instead of aborting mid-pipeline (the `tsc` model): every pass runs to completion and contributes its diagnostics. Codes are stable: `E001` (extraction), `A001–A008` (`graphql:` annotation resolution and projection modes — conflicts, targets, and values `A001–A003` are errors; recognition `A004` a warning; the mode notes `A006`/`A007` infos and the explicit-mode field omission `A008` a warning; `A005` is retired), `B001–B005` (build references — `B005` warns that two namespaces resolve to one prefix with no declaration in play), `V001–V016` (data/ontology validation), `M001–M006` (naming; `M003` is retired), `X002–X003` (union emission), `W001` (Relay wiring), `C001–C003` (composition). **Any error-severity diagnostic refuses the compile** — `compile()` throws `CompilationError` carrying the full diagnostic list. The rationale: an error diagnostic means the schema is missing something the ontology declared (a dropped field, an unregistered class, a failed extraction query), and a schema minus silently dropped fields must never be served — a boot dies loudly instead. Warnings and infos surface in `result.diagnostics` while the schema builds; the consumer chooses its policy for those.
