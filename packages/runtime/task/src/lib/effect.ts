...
/**
 * Options for a file copy. `content` supplies the source's contents when the
 * source cannot be read — the case a compiled binary is always in, where the
 * files were inlined at build time and `source` names a path under a virtual
 * filesystem that holds nothing.
 *
 * A COPY, not a write, on purpose: `WriteFile` is the effect a generated file
 * flows through, and hosts transform it (the summon stamp prepends a
 * "Generated by" comment to every `WriteFile`). Routing a verbatim asset
 * through `WriteFile` would silently stamp it — measured at 52 of
 * `@canonical/summon-application`'s 62 assets (28 `.ts`, 22 `.tsx`, 2 `.css`;
 * the rest have no comment syntax) — changing what a scaffold puts on disk.
 * Keeping the tag preserves the dry-run label, the undo, and the stamp
 * boundary; only the read is redirected.
 */
export interface CopyFileOptions extends UndoOptions {
  content?: string;
}
...
export const copyFileEffect = (
  ...
  source,
  dest,
  // Present only when supplied, so an ordinary copy's effect object is
  // structurally unchanged and every existing equality assertion still holds.
