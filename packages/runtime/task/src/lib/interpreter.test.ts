...
import { copyFileEffect } from "./effect.js";
import {
  executeEffect,
  run,
  runTask,
...
describe("CopyFile with pre-loaded content", () => {
  it("writes the supplied content instead of reading an unreadable source", async () => {
    const dir = mkdtempSync(join(tmpdir(), "task-copy-content-"));
    const dest = join(dir, "nested", "asset.tsx");
    await executeEffect(
      copyFileEffect("/$bunfs/root/templates/asset.tsx", dest, {
        content: "export const x = 1;\n",
      }),
      new Map(),
    );
    expect(readFileSync(dest, "utf-8")).toBe("export const x = 1;\n");
  });

  it("writes an EMPTY pre-loaded asset rather than falling back to the read", async () => {
    // `.gitkeep` is genuinely empty; keying off falsiness would try to read a
    // source path that does not exist and throw ENOENT.
    const dir = mkdtempSync(join(tmpdir(), "task-copy-content-"));
    const dest = join(dir, "assets", ".gitkeep");
    await executeEffect(
      copyFileEffect("/$bunfs/root/templates/assets/.gitkeep", dest, {
        content: "",
      }),
      new Map(),
    );
  ...
  it("still reads the source when no content is supplied", async () => {
    const dir = mkdtempSync(join(tmpdir(), "task-copy-content-"));
    const source = join(dir, "src.txt");
    writeFileSync(source, "from disk");
    const dest = join(dir, "out.txt");
    await executeEffect(copyFileEffect(source, dest), new Map());
    expect(readFileSync(dest, "utf-8")).toBe("from disk");
  });

  it("is not a WriteFile — a host stamp transform never sees it", () => {
    // The stamp boundary is the reason this stayed a CopyFile: summon prepends
    // a "Generated by" comment to every WriteFile, and a verbatim asset must
    // land byte-identical to its source.
    expect(copyFileEffect("a", "b", { content: "x" })._tag).toBe("CopyFile");
  });
});
