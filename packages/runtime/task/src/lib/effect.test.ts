...
describe("Effect Utilities - describeEffect", () => {
  ...
  // completions`: 2 short, for every shell, because the emitted script header
  // carries one em dash. The absolute size moves with the verb set (measured
  // against the current tree: bash 7990 code units / 7992 bytes, zsh 6878 /
  // 6880, fish 8550 / 8552); the delta of 2 does not. These two rows separate
  // the two readings, which is the invariant that outlives the numbers.
