...
describe("monorepo generator", () => {
  ...
  describe("shared utilities", () => {
    it("validates monorepo name", async () => {
      const { validateMonorepoName } = await import("../shared/index.js");
    ...
    it("validates repository URL", async () => {
      const { validateRepository } = await import("../shared/index.js");

      expect(validateRepository("https://github.com/org/repo")).toBe(true);
      expect(validateRepository("")).toBe(true); // Optional
      expect(validateRepository("not-a-url")).not.toBe(true);
    });

    it("creates template context", async () => {
      const { createTemplateContext } = await import("../shared/index.js");
