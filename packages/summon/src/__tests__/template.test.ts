...
describe("Template - templateHelpers", () => {
  describe("camelCase", () => {
    ...
    it("handles multiple separators", () => {
      expect(templateHelpers.camelCase("my-component_name")).toBe("myComponentName");
    });
  ...
  describe("kebabCase", () => {
    ...
    it("handles mixed case with lowercase-uppercase boundaries", () => {
      ...
      expect(templateHelpers.kebabCase("parseXMLDocument")).toBe("parse-xmldocument");
    });

    it("splits on each lowercase to uppercase transition", () => {
      // "parseXmlDocument" has e→X and l→D transitions
      expect(templateHelpers.kebabCase("parseXmlDocument")).toBe("parse-xml-document");
    });
  });
...
describe("Template - generatorComment", () => {
  ...
  it("includes timestamp when specified", () => {
    ...
    expect(result).toMatch(/^\/\/ Generated by ComponentGenerator on \d{4}-\d{2}-\d{2}/);
  });
  ...
  it("includes both version and timestamp", () => {
    ...
    expect(result).toMatch(/^\/\/ Generated by ComponentGenerator v2\.0\.0 on \d{4}/);
  });
...
describe("Template - generatorHtmlComment", () => {
  ...
  it("includes version when specified", () => {
    const result = generatorHtmlComment("ComponentGenerator", { version: "1.0.0" });
    expect(result).toBe("<!-- Generated by ComponentGenerator v1.0.0 -->");
  });

  it("includes timestamp when specified", () => {
    const result = generatorHtmlComment("ComponentGenerator", { timestamp: true });
    expect(result).toMatch(/^<!-- Generated by ComponentGenerator on \d{4}-\d{2}-\d{2}/);
  });
