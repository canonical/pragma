...
- **The parity flag set** — what the conformance matrix holds both help pages
  ...
  nothing, where pragma with the same argv writes), and its
  \`--no-generated-stamp\` changes the generated bytes. The completion
  pair is the sharpest case — the one summon extra with an effect
  OUTSIDE the target tree: read by summon's own whole-argv scan BEFORE
  Commander parses, appended to a complete leaf invocation either one
  suppresses the scaffold entirely, so
  ...
  \`error: unknown option '--detail=summary'\`, exit 2, nothing written,
  ...
  same gate), where summon with each of the three exits 0 and writes
  the tree. Commander's
