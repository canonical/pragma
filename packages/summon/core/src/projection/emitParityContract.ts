...
- **The parity flag set** — what the conformance matrix holds both help pages
  ...
  generated bytes. The SHORT forms are summon-only SPELLINGS, of
  parity-set flags included: \`-d\` (\`--dry-run\`) and \`-y\` (\`--yes\`)
  alias two of the mutation trio, \`-v\`/\`-l\` alias summon extras — so
  \`summon … -y\` writes a tree where \`pragma create … -y\` is
  \`error: unknown option '-y'\`, exit 2, nothing written. Commander's
