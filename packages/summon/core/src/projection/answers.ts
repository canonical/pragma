...
export function hasAllRequiredAnswers(
  ...
  for (const prompt of prompts) {
    ...
    if (!Object.hasOwn(answers, prompt.name) && prompt.default === undefined) {
      return false;
    }
  }
  return true;
...
export function applyDefaults(
  ...
  for (const prompt of prompts) {
    if (!Object.hasOwn(result, prompt.name) && prompt.default !== undefined) {
...
export function explicitAnswersComplete(
  ...
  for (const prompt of prompts) {
    if (isConditional(prompt)) continue;
    if (!Object.hasOwn(explicit, prompt.name)) return false;
  }
  return true;
}
...
export function pendingPrompts<P extends PromptLike>(
  ...
  return prompts.filter((prompt) => !Object.hasOwn(explicit, prompt.name));
}
