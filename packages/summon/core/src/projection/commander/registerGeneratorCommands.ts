...
export default function registerGeneratorCommands<G extends SurfaceGenerator>(
  ...
  for (const entry of barrel) {
    ...
    // Already registered (a namespace + runnable at the same path): a
    // runnable entry upgrades the existing namespace command into a FULL
    // leaf — the declared positional (the namespace spec carried none, so
    // Commander must learn the argument or the action would mis-slot its
    // arguments), the leaf usage line, and the host's standard flags —
    // exactly what the fresh-registration path below gives a leaf. A
    // duplicate namespace entry is skipped.
    ...
    if (existingCmd) {
      if (entry.generator) {
        ...
        if (positionalPrompt) {
          ...
          existingCmd.argument(`[${positionalName}]`);
          existingCmd.usage(`[${positionalName}] [options]`);
        }
        host.registerFlags(existingCmd);
        configureGeneratorCommand(
          existingCmd,
          entry,
          barrel,
          host,
