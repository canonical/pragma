 ...
 * an entry outside `dist/`, e.g. a source-served config package), which the
 * gate skips.
 *
 * The entry (`module` ?? `main`) alone was the whole freshness subject once,
 * and that was too narrow: the suites also consume `types` (a dependent's
 * tsc reads summon-core's `dist/types`), the export-map subpaths
 * (`@canonical/summon-core/projection` resolves into `dist/esm/
 * projection/`), and the template copier's destination (this package's own
