...
if (import.meta.main) {
  ...
  // `as const` is load-bearing: `Bun.build`'s `compile.target` is a literal
  // union, so a `Record<string, string>` lookup widens to `string` and does not
  // type-check. (It went unnoticed while `scripts/` sat outside tsconfig's
  // `include`; main widened `include`, which is what surfaced it.)
  const HOST_TARGETS = {
  ...
  const compileTarget =
    HOST_TARGETS[
      `${process.platform}-${process.arch}` as keyof typeof HOST_TARGETS
    ] ?? "bun-linux-x64";
