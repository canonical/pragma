...
// Regenerate: \`bun run scripts/build.ts\`. Inlines the template tree of every
// root DECLARED in \`create/constants.ts\` (all file types, not just .ejs) as
// strings. Only \`component\` declares one: \`loadTemplateSync\` reads the real
// file from the resolved generator package FIRST and reaches this manifest only
// when that read throws, so it is a fallback for a broken install, not a
// delivery mechanism. Keys follow summon-core's qualified-key scheme.
