...
import type { PromptCliSyntax } from "../src/capabilities/create/types.js";
...
export function generateCreateSurface({
  ...
  for (const [kind, binding] of Object.entries(CREATE_GENERATORS)) {
    for (const commandPath of binding.paths) {
      ...
      const projected = projectGenerator(
        commandPath.split("/"),
        generator as never,
      );
      entries[commandPath] = projected;
      // The REGISTERED spelling of each prompt, derived HERE — the build is
      // the one place allowed to call the projection's flag-shape authority —
      // so the shipped fast paths read baked tokens instead of importing the
      // registration machinery to re-derive them.
      const perPrompt: Record<string, PromptCliSyntax> = {};
      for (const prompt of projected.prompts) {
        const info = buildOptionInfo(prompt);
        perPrompt[prompt.name] = {
          flag: info.flags.split(" ")[0] as string,
          takesValue: info.flags.includes("<"),
          kebabName: info.kebabName,
        };
      }
      syntax[commandPath] = perPrompt;
    }
  }

  const stringifyRecord = (record: Record<string, unknown>): string =>
    Object.keys(record)
      .sort()
      .map(
        (key) =>
          `  ${JSON.stringify(key)}: ${JSON.stringify(record[key], null, 2)
            .split("\n")
            .join("\n  ")},`,
      )
      .join("\n");
  const body = stringifyRecord(entries);
  const syntaxBody = stringifyRecord(syntax);
