...
The concrete verbs live under `src/capabilities/` as `CapabilityModule`s, collected into one array. A directory exists there only where its noun needs hand-written code — the kernel's own nouns (`info`, `config`, `sources`, `doctor`, `graph`, `ontology`, `prompt`, `skill`, `setup`, `upgrade`, `create`, `capabilities`, `colophon`, `meta`), and two DOMAIN verbs the story grammar cannot express: `block list` (a tier-chain and channel filter the grammar has no term for) and `tier lookup` (frozen by the covenant with a single positional). Every other design-system read noun is a **declarative story**: the distribution's own five are declared as data in `pragma.conf.ts` and compiled at module load through the same compiler a third-party story goes through.
