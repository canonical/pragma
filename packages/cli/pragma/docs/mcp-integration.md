...
The freeze is enforced, not merely asserted, and it takes **two** guards because one cannot see the whole of it. `surface/covenant.json` records the freeze in its `$comment`; `capabilities/resources/resources.test.ts` derives ONE scheme token from that covenant and holds all four writings to it — the covenant entry, the provider's declared template, every URI the listing mints (the recovery entry included), and both `_meta` key namespaces. That catches the mutation that was actually made: one writing changed, three left behind.
