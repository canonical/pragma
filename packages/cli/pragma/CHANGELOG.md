...
* **cli:** a noun or verb named after a shell reserved word (`esac`, `in`, `do`, `done`, `fi`, …) fails at emit and at build, naming the token. It passed the injection allowlist and produced a script bash AND zsh refuse to parse, so `_pragma` was never defined and the user got no completion at all — silently, from a valid-looking installed file.
