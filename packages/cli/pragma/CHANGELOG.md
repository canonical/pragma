...
| bare `pragma create component` in a pipe wrote a full scaffold | REFUSES with recovery (exit 2) unless `--yes`, `--dry-run`, or complete flags | A non-TTY mutation without complete explicit input refuses in BOTH CLIs (the summon bin adopts the same decision). The MCP plan-first flow is unchanged. |
