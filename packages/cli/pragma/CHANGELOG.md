...
This release reshapes the command surface. Migrate as follows:
...
| `pragma tokens …` / `tokens_*` tools | `pragma token …` / `token_*` tools | The token noun and its tools are singular now: `token list`, `token lookup`, `token sample` — the three verbs are the whole replacement. |
...
| `config show`'s identity rows | `pragma --help` / `pragma colophon` | `name`, `help`, `colophon` and `issuesUrl` are read from the distribution config at module load — none of the four is merged through the layers — so `config show` no longer prints them, and `config show --format json` drops them from `data.config` and `data.origins`. A global or project layer declaring `name`, `help` or `issuesUrl` is still accepted by the validator and, as before, has no effect; it is now silent instead of being reported with a `[project]` marker nothing honours. `colophon` is the exception, because its SHAPE changed too: a higher layer may declare the new `{ markdown, summary? }` form (accepted, ignored), while the old byline string is rejected outright — see the `colophon:` row below. |
| `covenant.json`'s `configFiles.lock` | `configFiles.configCache` | The key named the project-lock file this release removes; it always described the evaluated-project-config cache. The value is unchanged. `surface/covenant.json` has no consumer outside the CLI package. |
| `colophon: "<byline>"` | `colophon: { markdown, summary? }` | The distribution's colophon is declared CONTENT now, not a byline the validator accepted and nothing read: `pragma colophon` renders it as the first section, titled with the binary's name, ahead of each active pack's own. Both values are Markdown bodies with no leading heading — the renderer supplies one — and `summary` is what `--format llm` emits, so a distribution that omits it hands an agent the full body. A layer declaring the old string form now throws `CONFIG_ERROR` at startup instead of being accepted and ignored, and the error names the file and the edit rather than reporting a type mismatch. In the new shape it still has no effect outside the distribution layer, which is where the rendered colophon is read from — so a global or project layer's error asks you to DELETE the line, and only the distribution config's asks for the new shape. |
| `colophon`'s `"kind": "pragma"` | `"kind": "distribution"` | The JSON section discriminant named this distribution in a field no content could reach, so a fork's `colophon --format json` reported `"pragma"` whatever it was called. Any client that persists the `colophon` tool's payload and branches on `kind` must read the new value; there is no alias. The MCP resource scheme is deliberately NOT treated this way — see `docs/mcp-integration.md`: `kind` is content a distribution authors about itself, while a resource URI is an address a client has already stored. |
| `completion.caseSensitive` | — | The field was validated and read by nothing: `setup completions` reads only `completion.minChars` and `completion.families`, and no config layer has ever reached the completion path's case folding. It is removed. Because unknown keys are stripped for forward compatibility, a config still declaring it would have gone on succeeding silently, so it now throws `CONFIG_ERROR` at startup naming the file and the field. Delete the line; there is no replacement to write anywhere. Completion matching is case-insensitive throughout, and nothing a config layer or a declared story can say changes that: the only live switch is `caseSensitive` on a hand-authored capability's completion heuristic, which the pack grammar has no field for and every shipped capability leaves at its insensitive default. |
| `--dry-run` / MCP plan-first reported a plan that could not happen | the same flags, now reading for real | A preview used the node-free MOCKING interpreter: `readFile` answered `[mock content of <path>]` and `exists` answered from a virtual set that starts empty. A mutation whose next effect depends on what it just read therefore planned a DIFFERENT branch than it ran, and one whose real run dies on a read planned a full success and exited 0. Both surfaces now use `planTask`, which performs reads (`ReadFile`, `Exists`, `Glob`, `TransformFile`'s read half, the context effects) against the real filesystem and simulates only the destructive effects. Two consequences to expect: a plan can now FAIL — with the same error and exit code the real run would produce — and a plan's reported effects and byte counts change to the real ones (measured: `config set tier apps/lxd` over an existing global config planned a 25-byte write that would have deleted the user's other settings, and now plans the 78-byte merge the run performs). Nothing to edit; if a `--dry-run` starts failing, the run it previews was already failing. A `create` preview's byte counts change again for a second reason: the generated-file stamp rides the same effect seam the real run stamps on, and the plan branch was not applying it, so every planned file was short by exactly its stamp (measured: `create package --name @acme/my-lib` planned `src/index.ts (64 bytes)` / `README.md (253)` where the run writes 114 / 309). `Exec` is still simulated, copy sources are still unprobed, `Glob` does not see what the plan would have written, a copied or `mkdir`-ed path exists to the plan but has no readable bytes, a simulated delete is not subtracted from what a later `exists` sees, and a simulated write/`mkdir`/copy cannot FAIL where its real counterpart does — every one of them is enumerated in `@canonical/task`'s `lib/plan.ts`, which is the single writing this row defers to rather than carrying its own count. A plan's reported byte counts are also now UTF-8 bytes rather than UTF-16 code units, so a write carrying a non-ASCII character reports what the run performs (measured on `setup completions`, whose emitted script carries one em dash: the plan read two short, the em dash's three UTF-8 bytes against its one UTF-16 unit). |
| a third-party story's unbound prefix → `STORE_UNAVAILABLE` (exit 3) | → `CONFIG_ERROR` (exit 1) | A story declared in your config or shipped by a package, whose query names a prefix the built store does not bind, was told the store was not built and to run `pragma sources update` — a lever that cannot help, because rebuilding cannot bind a prefix nothing declares. It is now classified as what it is: a declaration error naming the story, with no recovery command. The DISTRIBUTION's own stories are unchanged — for them an unbound prefix does mean the store was never built, and they keep `STORE_UNAVAILABLE` with the `sources update` recovery. A script branching on exit 3 for this case must branch on 1; the four-code classification set is unchanged. |
| `detail: <any string>` | `detail: summary` \| `standard` \| `detailed` | `detail` was documented as an enum and validated as a free string, so `detail: "banana"` passed, was reported as `[project]` by `config show`, and silently rendered at `standard`. It is now validated: a layer declaring any other value fails at load — every command, `doctor` and `sources update` included — with a `CONFIG_ERROR` naming the file, the field, the three levels and the edit. Change the value BY HAND — `config set detail <level>` reads the config layers before it writes, so from the broken state it fails with the same error. There is nothing else to migrate. |
| `surface/surface.v2.json` | `surface/covenant.json` | The frozen surface document is renamed. The file names what it IS — the covenant every writing about it already called it — instead of a version scheme this CLI does not use. Its contents are unchanged apart from the `$comment`, which no longer opens on that scheme and no longer carries the per-PR tool-count archaeology. Nothing outside this package reads the file, so there is nothing to migrate; it is recorded because the document is a published contract and a covenant edit is never silent. There is no alias and no symlink. |
| `pragma token add-config` / the `token_add-config` tool | — | Removed, with no replacement. It wrote a starter `tokens.config.mjs` for the terrazzo pipeline; the declared-content grammar the read surface is now built from has no verb for mutations, so keeping it meant keeping the last hand-written data command in the kernel. Write the file by hand: it is a terrazzo `defineConfig` whose `tokens` array holds one `node_modules/<pack-name>/tokens/**/*.json` glob per configured pack (or `node_modules/@canonical/ds-global/tokens/**/*.json` when none is configured), with `outDir: "./tokens/"` and no plugins. The covenant tool count changes 38 → 37. `token list`, `token lookup` and `token sample` are unchanged. |
| `pragma tier lookup <name>` | `pragma tier lookup <name...>` | The tier noun is declared content now — the last hand-written read verb is gone — so its lookup is the same compiled lookup every other noun has, and four things change with it. The positional is VARIADIC (`tier_lookup { name: ["apps/lxd"] }` over MCP, where it was a single string), and it accepts prefixed names, IRIs and globs like any other lookup. The **blocks scoped to the tier are no longer returned**: that field needed the INVERSE of `ds:tier`, which the story grammar admits in neither source, and there is no declared form to keep it in — a block names its own tier, so `pragma block lookup <block>` still answers the question from the other side. What the lookup returns is the tier's identity: `name` and `uri`, the two facts a tier carries in the graph. `name` is now the graph's own value (`Apps/LXD`), not the string you typed. Output moves to the shared lookup layout, so `--format plain` is the title over a rule and `--format llm` is an H2, where the bespoke renderer printed an IRI line and a blocks line; `--format json` gains the standard `{ results, errors }` envelope in place of the bare object. TAB completion now offers the index's compacted IRIs (`ds:apps_lxd`) rather than the display names (`Apps/LXD`) — both still resolve, and this is how every other noun already completes. |
| `pragma block list` filtered by tier and channel | `pragma block list` lists EVERY block | **The largest user-visible change in this release, and it is deliberate.** `block list` was the one hand-written read command: it narrowed rows to the configured tier's parent chain (`apps/lxd` → `global`, `apps`, `apps/lxd`) and to the release levels the configured channel admits, with `--all-tiers` as the escape. The read surface is declared content now, and the story grammar has a term for neither filter, so both are GONE rather than reimplemented — and `--all-tiers` goes with them, because there is nothing left to widen. `block list` returns every block in the graph: every tier, untiered blocks included, and every release level, **experimental and alpha included, for everyone**. Filtering will return in declared form; until it does, narrow the rows yourself or write the SELECT with `pragma graph query`. Measured against the pack this binary ships: the DEFAULT view is unchanged at 251 rows (that graph annotates no block with `ds:release`, and every block carries a tier), so nothing moves until you set a tier — `config set tier apps/lxd` used to yield 166 rows and now yields 251. A script passing `--all-tiers`, or an agent sending `block_list { allTiers: true }`, must drop the argument: an unknown flag is an error. One output shape moves with the verb: `--format llm`'s heading is `## Block (251)` where the hand-written renderer wrote `## Blocks (N)` — a declared list derives its heading from the noun, so this is the form `token`/`modifier`/`standard`/`tier` already had. An agent or script keyed on the plural string must key on the singular. |
| `config set tier` scoped your reads | `config set tier` changes nothing you can observe | Follows from the row above. `tier` is still validated, still stored, still reported by `config show` and `info` with its provenance — and now read by NO verb, because `block list` was its only reader. `channel` keeps exactly one reader, and it is a different meaning of the word: `info`'s update check resolves the matching npm dist-tag. Neither field is removed, because filtering is expected back in declared form and removing a field users have set would cost them the setting; but until then, setting either is a no-op for every read. The generated [configuration reference](./docs/reference/config.md) says so on both rows, and the MCP handshake no longer tells agents to scope data with them. |
| `covenant.json`'s `bins.pragma` describing a version | the same string without it | The frozen surface's `bins` entry read `pragma v2 CLI and MCP server host (stdio)`. It is a human description of the binary, not an identifier, and it named a version scheme this CLI does not use — so the `v2` is gone from both the covenant and `FIXED_SURFACE`, which the conformance deep-equals. Recorded because the covenant is a published contract and a covenant edit is never silent; nothing reads the string programmatically. The npm `description` of `@canonical/pragma-cli` is rewritten for the same reason: it read *"Next-generation CLI and MCP kernel for Canonical's design system (pragma v2)"*, naming both a predecessor and a version scheme that were ruled out. |
| `pragma ontology show` / the `ontology_show` tool | `pragma ontology lookup` / `ontology_lookup` | The by-name ontology read had TWO spellings. `show` was a self-declared deprecated alias of `lookup` — the same params, the same `--properties`/`--full-uris`/`--class` flags, the same disclosure ladder, the same formatters and a reference to the same handler; its own test asserted the two produced byte-identical output. It is removed with no shim, so `pragma ontology show ds` now exits non-zero with `Unknown command "show"`. Rename the call: `pragma ontology lookup ds`, or `ontology_lookup { prefix: "ds" }` over MCP. Nothing else about the read changes. The covenant tool count changes 37 → 36. |
| `covenant.json`'s `mcp` noun block | — | The covenant advertised an `mcp` noun with a `serve` sub-verb. **No such command has ever existed in code**: `bin.ts` answers `mcp` at argv[0] before the command tree is built, and the two `hidden: true` specs that nominally declared the noun were filtered out by `buildProgram` and never registered, so `pragma mcp serve` was never a thing a user could run and `pragma mcp` reached the bin fast path regardless. The block is removed rather than implemented; conformance could not have caught it, because it only walks EMITTED nouns and this noun emitted nothing. There is nothing to migrate — keep running `pragma mcp`, which is unchanged. |
| a story declaring noun `mcp`, `__complete` or `__store-probe` | delete the story | Follows from the row above, and it is the half that costs an edit. These three tokens are answered by `bin.ts` at `argv[0]`, before the command tree is built, so a story behind one of them registered and then answered nothing — permanently, with no way to reach it. In your CONFIG that is now a fatal `CONFIG_ERROR` at startup — every command, `doctor` included — naming the noun and why it can never run, where before every command still worked and only the noun was dead. Delete the story; there is no replacement noun to move it to. A story shipped by a PACKAGE in `stories/*.json` is not fatal: as with any other invalid package story it is dropped and named on stderr and under `doctor`'s `pack refs`, because a package's file is not the user's to edit. |
| `capabilities --format json`'s `limits.condensed_available` | — | The field was a permanent literal `false` that CONTRADICTED the same binary's `--format` help ("llm — condensed Markdown for agents"), in the very payload an agent reads to learn what output it can ask for. `llm` IS the condensed mode, it is already named in `limits.output_modes`, and it is auto-selected when stdout is not a TTY — so the field said no to something on by default. Removed rather than flipped to `true`, because `output_modes` already carries the fact. This is a wire-visible removal: the `capabilities` MCP tool returns the same payload, so a client branching on `limits.condensed_available` now reads `undefined`. Branch on `limits.output_modes.includes("llm")` instead, which was always the honest test. |
| `info --format json`'s `updateSkipped` | — | The boolean was computed, shipped and asserted, and reached NO renderer in any of the three formats — `plain`, `llm` and `json` all print an `update` line or nothing — so the offline case and the up-to-date case were already indistinguishable to every human reader. Only a JSON or MCP consumer could see it at all. It is removed with no replacement: `data.update` is present when a newer version was resolved and absent otherwise, which is the same two-way distinction the field was trying to make a three-way one. A script branching on `data.updateSkipped` now reads `undefined`; branch on the presence of `data.update`. |

See [docs/getting-started.md](./docs/getting-started.md) for the current workflow and [docs/reference/index.md](./docs/reference/index.md) for the full command and tool surface.
...
* **cli:** fish stops spawning a process on a purely structural `--<TAB>`. fish evaluates every `complete` rule matching a position (bash and zsh route one exclusive `case` arm), so a positional's `__complete` delegation fired alongside the flag-name rules and its reply landed in the user's candidate list. All three shells now answer structure with zero exec.
* **cli:** a noun named `esac` fails at emit and at build, naming the token. It passed the injection allowlist and produced a script bash AND zsh refuse to parse — it terminates the `case "$noun" in … esac` dispatch — so `_pragma` was never defined and the user got no completion at all, silently, from a valid-looking installed file. `esac` is the only such word: all 17 of bash's reserved words were rendered into the live templates and checked with real bash 5.2.21 and zsh 5.9, and the other 16 parse cleanly as nouns, as do all 17 as verb labels.
* **cli:** a `stories.json` that is non-empty but not a JSON array no longer counts as a complete pack. `buildPack` reused the directory, the boot served it, every package-declared noun disappeared, and `sources update` reported success.
* **cli:** the boot decision reads a pack manifest without zod, taking the schema off the storeless fast path (`__complete` netted 24–26 ms → 19–21 ms on the compiled binary).
