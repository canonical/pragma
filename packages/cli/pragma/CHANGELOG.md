...
| `create application --with-ssr/--with-router/--with-forms/--with-relay` | `create application react` with `--no-forms`/`--relay` — SSR and the router are always on | The B8 `--with-X` aliases die: the generator's own prompt names ARE the flag names, and `react` is the tree segment. The ssr/router pair is gone WITH its prompts: the scaffold has no SPA arm, so the two questions only ever accepted their default — `--no-ssr`/`--no-router` are unknown options, and the wizard no longer asks them. |
