...
| `completion.caseSensitive` | — | The field was validated and read by nothing: `setup completions` reads only `completion.minChars` and `completion.families`, and completion matching is always case-insensitive. It is removed. Because unknown keys are stripped for forward compatibility, a config still declaring it would have gone on succeeding silently, so it now throws `CONFIG_ERROR` at startup naming the file and the field. Delete the line. A pack STORY's `complete.caseSensitive` is a different, live field and is untouched. |
