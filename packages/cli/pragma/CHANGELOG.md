...
* **cli:** `generators` returns to `pragma.conf.ts`, load-bearing. It went away INERT (validated, layered, read by nothing) and comes back read at BUILD time: `scripts/build.ts` writes the literal import specifiers a `--compile` bundle needs, harvests each declared package's template roots, and derives the whole `create` surface — prompt mirrors, framework axis, path params, help text, summaries and examples — from it. `src/capabilities/create/` names no generator package at all. An entry is `{ name, nouns }`: the name is the specifier the build writes AND the `package.json#dependencies` key it is checked against, because the dependency is what installs and links, and a `source` restating that range could only disagree with it. Each noun declares `key` alone or `keyPrefix` + `axis` — never a mixture — plus `docs` (help text the build cannot derive from a wizard question, including the axis flag's own, which mirrors no prompt) and, where the name heuristic does not reach it, the `pathParam` its workspace jail covers; a positional the jail would not cover is a build error, not a silent absence. Distribution-only, like the identity fields: a global or project layer declaring it is accepted and ignored, because no config layer can change which modules an already-compiled binary carries. A declared package must be linked into this package's `node_modules` and must expose its `src/**/templates` tree — the manifest is harvested from source at build time, so a `dist`-only published tarball does not satisfy it and a fork vendors its generator package as a workspace sibling.
...
| `generators:` in a config | `generators:` in the DISTRIBUTION config | The v1 field was validated, layered, documented and read by nothing, and it was removed as inert — from the schema, from `config show`, and from the public type barrel (`GeneratorSource`). It has since RETURNED, load-bearing and read at BUILD time: `pragma.conf.ts` declares the generator packages the distribution links and the `create` nouns they expose, and `scripts/build.ts` turns that into the literal import specifiers a `--compile` bundle needs, the harvested template manifest and the whole `create` surface (see the current Unreleased entry for the shape). It is distribution-only, like the identity fields: a global or project layer declaring it is ACCEPTED and ignored — validated shallowly, never refused — because no config layer can change which modules an already-compiled binary carries. Changing it means editing `pragma.conf.ts` and rebuilding. `config show` still does not report it. |
