...
export const PARITY_GAPS: readonly ParityGapEntry[] = [
  ...
  },
  {
    ...
    area: "config setters",
    description:
      "`config set tier <anything>` is STORELESS and writes the value verbatim: the old store-backed `validateTier` (which queried the ontology for valid tier paths) is dropped, and the covenant's `config_set` declares no `needsStore` to bring it back. What changed with the tier scope is the CONSEQUENCE of a bad value — it used to yield nothing at all (the key scoped nothing), and it is now refused at the first tiered READ, with an INVALID_INPUT naming the tiers the pack does carry and a `config unset tier` recovery. Validation at the read rather than at the write is deliberate: the write has no store, and the pack a value is valid against can change under a machine that set it months ago. `config/fields.ts`, `kernel/packs/tierScope.ts`.",
  },
  {
