 ...
 *
 * Setting a tier activates a parent chain filter. Because the tier
 * hierarchy is nested (global > apps > apps/lxd), selecting a leaf
 * tier also makes all ancestor tiers visible. For example,
 * `"apps/lxd"` resolves to visible tiers: `global`, `apps`, `apps/lxd`.
 * When no tier is set, all tiers are visible (no filter applied).
 */
