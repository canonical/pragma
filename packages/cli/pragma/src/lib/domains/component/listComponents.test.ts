...
describe("listComponents", () => {
  ...
  it("returns empty modifiers for a component with no modifiers", async () => {
    const result = await listComponents(store, noFilter);
  ...
  it("returns tokenCount 0 for a component with no tokens", async () => {
    const result = await listComponents(store, noFilter);
    const card = result.find((c) => c.name === "Card");
    expect(card).toBeDefined();
    expect(card?.tokenCount).toBe(0);
  });

  it("returns empty array for an empty store", async () => {
    const empty = await createTestStore({ ttl: "" });
    const result = await listComponents(empty.store, noFilter);
