...
const setupAllVerb = setupVerb(
  ...
  SELF_CAPABILITY,
  {
    doc: "Plans every target in the selected band, then applies the ones you keep. The user/home band is the default; --local targets the project band, and --both runs each. A run with no terminal prints the plan and applies nothing unless --yes is given.",
    examples: [
      { cmd: `${BIN_NAME} setup` },
      {
