...
describe("declared generator bindings (PROTECTED)", () => {
  ...
  it("the framework enum offers exactly the declared frameworks, with no default", () => {
    const framework = createVerbs.component.params.find(
      (p) => p.name === "framework",
    );
    // An empty `values` would reject every framework while `--help` still
    // advertised the positional — the hazard this names. (That every value
    // resolves to a generator is the test above.)
    expect(framework?.kind).toBe("enum");
    const values = (framework as { values: readonly string[] }).values;
    expect(values.length).toBeGreaterThan(0);
    expect(values).toEqual([...CREATE_GENERATORS.component.frameworks]);
    // And NO default. `create component` mirrors `summon component`, where the
    // framework is a required POSITIONAL selector; a default is exactly what
    // made an omitted framework silently scaffold React. `pickGenerator.test.ts`
    // pins the refusal, this pins the declaration it comes from — together they
    // keep the enum derived from `CREATE_GENERATORS` AND undefaulted.
    expect(framework).not.toHaveProperty("default");
  });
