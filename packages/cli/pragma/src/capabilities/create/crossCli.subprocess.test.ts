 ...
 * options sections of the two `--help` pages must agree STRUCTURALLY below
 * the host blocks — same groups, rows, order, defaults; bytes stopped being
 * the surface when pragma took the help seam's presentation half
 * (`generatorStructure` carries the why). Wizard-script parity is asserted
 * on the projection
...
function helpOf(
  ...
  // Colour explicitly OFF: pragma's help renders through chalk, which
  // honours FORCE_COLOR / CI env vars even piped (nx sets FORCE_COLOR for
  // test tasks), and the structural cells below parse plain bytes. NO_COLOR
  // is the one convention pragma's style seam obeys unconditionally;
  // summon's shared renderer never colours, so its bytes are unaffected.
  // FORCE_COLOR is removed rather than merely overridden — Bun warns on
  // stderr when it sees both, and this helper folds stderr into the text.
...
/** One help section: its heading and its `[flags, description]` rows. */
interface HelpSection {
  readonly heading: string;
  readonly rows: readonly (readonly [flags: string, description: string])[];
}

/**
 * The STRUCTURE of a help page's generator-option sections: every section
 * below the host's standard-flag block, as heading + `[flags, description]`
 * rows, with the presentation collapsed — the trailing heading colon and the
 * column padding are each renderer's own. Presentation-independent on
 * purpose: byte parity of these sections ended when pragma took the help
 * seam's presentation half (`CommanderHost.renderHelp`) and began rendering
 * the shared structure in its house style — the library decides STRUCTURE,
 * the host effects PRESENTATION (the same split the usage errors pin) — so
 * the parity surface below the host block is the groups, the rows, their
 * order and their defaults, extracted here from each page's own bytes.
 */
function generatorStructure(help: string): HelpSection[] {
  const blocks = help
    .split(/\n{2,}/)
    .map((block) => block.split("\n").filter((line) => line !== ""));
  const sections = blocks
    // A section is a non-indented heading over indented rows; the usage and
    // description blocks are single lines and drop out here.
    .filter(
      (block) => block.length > 1 && !(block[0] as string).startsWith("  "),
    )
    .map((block) => ({
      heading: (block[0] as string).replace(/:$/, ""),
      rows: block.slice(1).map((row): [string, string] => {
        const match = row.match(/^\s+(.+?)(?:\s{2,}(.*))?$/);
        return [match?.[1] ?? row.trim(), match?.[2] ?? ""];
      }),
    }));
  // Host flags live above the parity surface by design (each host declares
  // its own standard block).
  const globalAt = sections.findIndex((s) => s.heading === "Global Options");
  return sections.slice(globalAt + 1);
}
...
describe("cross-CLI conformance matrix (PROTECTED)", () => {
  ...
  for (const fixture of CONFORMANCE_FIXTURES) {
    it(`${fixture.name}: help/flag parity over the parity set + structurally equal generator sections`, async () => {
      ...
      // The GENERATOR-OPTIONS sections agree STRUCTURALLY below the host
      // blocks (same buildOptionInfo, same groups, same rows, same order,
      // same defaults) — see `generatorStructure` for why bytes are no
      // longer the surface. Non-empty first, so a parser that extracted
      // nothing from both pages cannot green the cell vacuously.
      const pragmaStructure = generatorStructure(pragmaHelp);
      expect(pragmaStructure.length).toBeGreaterThan(0);
      expect(pragmaStructure).toEqual(generatorStructure(summonHelp));
    }, 60_000);
  }
