...
describe("`block list` orders its ties totally (PROTECTED)", () => {
   ...
   *
   * Asked for as ONE page over the whole population, because the order under
   * test is the order of the whole population. A default-paged read used to be
   * the same thing and quietly stopped being it: the shipped pack now carries
   * 313 blocks and `DEFAULT_LIST_LIMIT` is 300, so the tail of the alphabet —
   * `Timestamp`, `Tooltip`, and their launchpad twins — fell off the page and
   * the cases below read a missing row as a lost row. The page is asserted
   * COMPLETE rather than assumed so, so that the next time the population
   * outgrows the window this says which it was.
   */
