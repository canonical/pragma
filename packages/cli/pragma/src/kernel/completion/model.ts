...
/**
 * Words bash and zsh will not accept as a bare `case` pattern or a function-name
 * fragment. Each passes {@link SAFE_TOKEN_RE} — they are ordinary lowercase
 * words — and each breaks a generated script at PARSE time.
 *
 * `esac` is the sharp one: the bash and zsh templates dispatch on the noun with
 * `case "$noun" in … esac`, so a noun named `esac` closes the statement early
 * and the rest of the file is a syntax error. The failure is not a wrong
 * candidate — `_pragma` is never defined, so the user gets NO COMPLETION AT
 * ALL, silently, with a valid-looking installed file. `fish -n` does not catch
 * it either, because fish's script is `complete -c` lines with no `case`.
 */
...
/**
 * Assert a token may be INLINED into a shell script's grammar, not merely
 * printed as a candidate.
 *
 * Deliberately NOT folded into {@link assertSafeToken}, and deliberately NOT
 * called from {@link buildCompletionModel}. The two gates are no longer
 * identical, because the two hazards are not:
 *
 * - {@link assertSafeToken} guards INJECTION. It must run everywhere a token
 *   reaches a shell, including at runtime — `resolve.ts` filters `__complete`
 *   candidates through {@link SAFE_TOKEN_RE} before printing them.
 * - This guards a token being taken for GRAMMAR. That can only happen where a
 *   token is written INTO a script, i.e. in `emitScripts`. At runtime a
 *   reserved word is harmless: a candidate is printed, never evaluated.
 *
 * Keeping it out of the model matters for cost as well as clarity: `model.ts` is
 * on the `__complete` fast path (`complete.ts` imports it) and `emitScripts.ts`
 * is not, so this check costs that path zero.
 *
 * @param token - The candidate token (a noun or a verb label).
 * @param where - Human-readable location for the error message.
 * @throws Error when the token is a shell reserved word.
 */
export function assertShellUsableToken(token: string, where: string): void {
  if (SHELL_RESERVED_WORDS.has(token)) {
    throw new Error(
      `completion: reserved shell word ${JSON.stringify(token)} in ${where} — ` +
        "bash and zsh would refuse the generated script, leaving no completion " +
        "at all; rename it",
    );
  }
}
