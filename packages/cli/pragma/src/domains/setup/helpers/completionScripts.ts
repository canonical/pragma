 ...
 *
 * Each script delegates to `pragma --completions` at runtime
  ...
  switch (shell) {
    case "zsh":
      return [
        ...
        '  completions=("${(@f)$(pragma --completions "${words[@]}" 2>/dev/null)}")',
        "  compadd -- $completions",
        "}",
        "",
    ...
    case "bash":
      return [
        ...
        "  local completions",
        // biome-ignore lint/suspicious/noTemplateCurlyInString: shell syntax, not JS template
        '  completions=$(pragma --completions "${COMP_WORDS[@]}" 2>/dev/null)',
    ...
    case "fish":
      return [
        "# Generated by `pragma setup completions`",
        "",
        "complete -c pragma -f -a '(pragma --completions (commandline -cop) 2>/dev/null)'",
        "",
      ].join("\n");
  }
}
