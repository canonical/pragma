...
describe("formatters.plain", () => {
  it("includes block names", () => {
    const text = formatters.plain(components);
  ...
  it("returns empty string for empty list", () => {
    expect(formatters.plain([])).toBe("");
  });
});

describe("formatters.llm", () => {
  it("starts with heading", () => {
    const text = formatters.llm(components);
    expect(text).toContain("## Blocks");
  });

  it("bolds block names", () => {
    const text = formatters.llm(components);
    expect(text).toContain("**Button**");
    expect(text).toContain("**Card**");
  });
...
describe("formatters.json", () => {
  it("returns valid JSON array", () => {
    const text = formatters.json(components);
