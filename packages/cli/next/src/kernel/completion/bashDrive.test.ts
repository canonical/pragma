 ...
 * `_pragma2`, the value slot falls through to the noun/positional arm and
 ...
 * / COMP_CWORD, invoke `_pragma2`, and return COMPREPLY line-by-line.
 */
function driveBash(words: readonly string[], cword: number): string[] {
  const dir = mkdtempSync(join(tmpdir(), "pragma2-bashdrive-"));
...
describe.skipIf(!bashOk)("generated bash — --flag=value wordbreak (M1)", () => {
  ...
  it("still completes the space form --format <TAB> (unbroken)", () => {
    const reply = driveBash(["pragma2", "--format", ""], 2);
    expect(reply.sort()).toEqual(["json", "plain"]);
  });
});
